                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:0 0 16px 16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
                                <tr>
                                    <td style="background:linear-gradient(135deg,#0c4a6e 0%,#0369a1 50%,#0ea5e9 100%);padding:40px 48px;text-align:center;">
                                        ...
                                        <p style="margin:12px 0 0;color:rgba(255,255,255,0.8);font-size:13px;letter-spacing:2px;text-transform:uppercase;">New Rotations Found</p>
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    <td style="padding:24px 48px 40px;">
                                        <h1 style="margin:0 0 8px;color:#0c4a6e;font-size:24px;font-weight:700;text-align:center;">New Matching Rotations</h1>
                                        <p style="margin:0 0 24px;color:#64748b;font-size:15px;text-align:center;line-height:1.5;">Hi {{ $user->first_name }}, {{ $newSlots->count() }} new rotation(s) match your saved search <strong>"{{ $savedSearch->name }}"</strong>:</p>
                                        ...
                                        <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;margin-bottom:24px;">
                                            <tr style="background-color:#f8fafc;">
                                                <td style="padding:10px 16px;font-size:13px;font-weight:700;color:#475569;border-bottom:1px solid #e2e8f0;">Rotation</td>
                                                <td style="padding:10px 16px;font-size:13px;font-weight:700;color:#475569;border-bottom:1px solid #e2e8f0;text-align:right;">Dates</td>
                                            </tr>
                                            @foreach($newSlots as $slot)
                                            <tr>
                                                <td style="padding:12px 16px;font-size:14px;color:#1e293b;border-bottom:1px solid #f1f5f9;">
                                                    <strong>{{ $slot->title }}</strong>
                                                    <br><span style="font-size:12px;color:#94a3b8;">{{ $slot->specialty }} &bull; {{ $slot->site->name ?? 'N/A' }}</span>
                                                </td>
                                                <td style="padding:12px 16px;font-size:14px;color:#0369a1;font-weight:600;text-align:right;border-bottom:1px solid #f1f5f9;">
                                                    {{ $slot->start_date->format('M j') }} - {{ $slot->end_date->format('M j, Y') }}
                                                </td>
                                            </tr>
                                            @endforeach
                                        </table>

                                        <p style="margin:0 0 28px;color:#475569;font-size:15px;line-height:1.6;text-align:center;">View these rotations and apply before spots fill up!</p>
