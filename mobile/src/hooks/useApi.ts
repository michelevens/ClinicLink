...
/** React Native file pick result — replaces web `File` type */
...
export function useUploadCredentialFile() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, file }: { id: string; file: RNFile }) => studentApi.uploadCredentialFile(id, file),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['credentials'] }) },
  })
}
...
export function useUploadTaskFile() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ taskId, file }: { taskId: string; file: RNFile }) => onboardingTasksApi.uploadFile(taskId, file),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['onboarding-tasks'] })
      qc.invalidateQueries({ queryKey: ['onboarding-progress'] })
    },
...
export function useUploadAgreementDocument() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, file }: { id: string; file: RNFile }) => agreementsApi.uploadDocument(id, file),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['agreements'] }) },
  })
}
...
export function useUploadAgreementTemplateDoc() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, file }: { id: string; file: RNFile }) => agreementTemplatesApi.uploadDocument(id, file),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['agreement-templates'] }) },
  })
}
