// Mobile API — re-exports client from ./client.ts
// All API namespaces and types below are shared with the web frontend
...
export const studentApi = {
  ...
  uploadCredentialFile: (id: string, file: { uri: string; name: string; type: string }) => {
  ...
  downloadCredentialUrl: (id: string) =>
    api.getDownloadUrl(`/student/credentials/${id}/download`),
}
...
export const certificatesApi = {
  ...
  getPdfUrl: (id: string) =>
    api.getDownloadUrl(`/certificates/${id}/pdf`),
...
export const agreementsApi = {
  ...
  uploadDocument: (id: string, file: { uri: string; name: string; type: string }) => {
    const formData = new FormData()
    formData.append('file', file as unknown as Blob)
...
export const onboardingTasksApi = {
  ...
  uploadFile: (taskId: string, file: { uri: string; name: string; type: string }) => {
    const formData = new FormData()
    formData.append('file', file as unknown as Blob)
...
export const ceCertificatesApi = {
  ...
  downloadUrl: (id: string) =>
    api.getDownloadUrl(`/ce-certificates/${id}/download`),
...
export const accreditationReportsApi = {
  ...
  downloadUrl: (id: string) =>
    api.getDownloadUrl(`/accreditation-reports/${id}/download`),
  delete: (id: string) => api.delete(`/accreditation-reports/${id}`),
}

// --- Exports ---
// On mobile, all export URL builders are async (token from SecureStore)
export const exportsApi = {
  hourLogsCsvUrl: async (params?: { status?: string; slot_id?: string; date_from?: string; date_to?: string }) => {
    const qs = new URLSearchParams()
    if (params?.status) qs.set('status', params.status)
    if (params?.slot_id) qs.set('slot_id', params.slot_id)
    if (params?.date_from) qs.set('date_from', params.date_from)
    if (params?.date_to) qs.set('date_to', params.date_to)
    const base = await api.getDownloadUrl(`/exports/hour-logs/csv`)
    return `${base}&${qs}`
  },
  hourLogsPdfUrl: async (params?: { status?: string; slot_id?: string; date_from?: string; date_to?: string }) => {
    const qs = new URLSearchParams()
    if (params?.status) qs.set('status', params.status)
    if (params?.slot_id) qs.set('slot_id', params.slot_id)
    if (params?.date_from) qs.set('date_from', params.date_from)
    if (params?.date_to) qs.set('date_to', params.date_to)
    const base = await api.getDownloadUrl(`/exports/hour-logs/pdf`)
    return `${base}&${qs}`
  },
  evaluationsCsvUrl: async (params?: { type?: string }) => {
    const qs = new URLSearchParams()
    if (params?.type) qs.set('type', params.type)
    const base = await api.getDownloadUrl(`/exports/evaluations/csv`)
    return `${base}&${qs}`
  },
  evaluationsPdfUrl: async (params?: { type?: string }) => {
    const qs = new URLSearchParams()
    if (params?.type) qs.set('type', params.type)
    const base = await api.getDownloadUrl(`/exports/evaluations/pdf`)
    return `${base}&${qs}`
  },
  complianceCsvUrl: async (params?: { site_id?: string }) => {
    const qs = new URLSearchParams()
    if (params?.site_id) qs.set('site_id', params.site_id)
    const base = await api.getDownloadUrl(`/exports/compliance/csv`)
    return `${base}&${qs}`
  },
  compliancePdfUrl: async (params?: { site_id?: string }) => {
    const qs = new URLSearchParams()
    if (params?.site_id) qs.set('site_id', params.site_id)
    const base = await api.getDownloadUrl(`/exports/compliance/pdf`)
    return `${base}&${qs}`
  },
}
